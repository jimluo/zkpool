...
use snarkvm::{
	...
	prelude::{
		Address, AleoID, CanonicalDeserialize, CoinbasePuzzle, EpochChallenge, PrivateKey,
		PuzzleConfig, Testnet3,
	},
	utilities::serialize::*,
};
use std::{
...
impl Prover {
	...
	fn setup_prove(degree: u32) -> (Box<CoinbasePuzzleInst>, u32) {
		...
		let prover =
			CoinbasePuzzleInst::trim(&universal_srs, config).expect("Failed to puzzle srs trim");
